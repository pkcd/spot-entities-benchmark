...
public class Benchmark {
    ...
    private final File entityFile; // from a source like aida_means.tsv
    private final File documentFile; // from a source like CoNLL.tsv

    public Benchmark(File eFile, File dFile) throws IOException {
        // this.entityFile = File.createTempFile(eFile.getPath(), "unique");
        // diego: not found exception with getPath.
        this.entityFile = File.createTempFile(eFile.getName(), "unique");
        ExternalSort.sort(eFile.getPath(), this.entityFile.getPath(), '\t', 1,
                true, false, false);
